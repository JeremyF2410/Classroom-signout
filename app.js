...
function render(){
  ...
  if(active.length===0){
    ...
    active.forEach(function(item){
      ...
      var time=document.createElement("div");time.className="out-time";addText(time,elapsed+" min");
      var destination=document.createElement("div");destination.className="out-destination";addText(destination,item.destination);
      card.appendChild(name);card.appendChild(time);card.appendChild(destination);outGrid.appendChild(card);
    });
  }
